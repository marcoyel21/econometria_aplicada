...
En resumen, solamente en una variable de las 3 tablas anteriores encontramos una difrerencia de medias estadísticamente significativa (computerskills). Sin embargo, las medias son muy parecidas (.809 para el control y .832 para el tratamiento). Es decir que esta mínimamente favoreciendo a los nombres negros. Fuera de ese detalle, todo lo demás parece indicar que la asignación de nombres fue aleatoria. 
...
+ **OLS con errores heterocedásticos**
...
stargazer(ols_con_controles,type="latex",summary=FALSE,header = F)
```
...
kable(modelos_output, col.names = c("Neyman","Robust OLS","Controled OLS","AME Probit"), longtable=T, booktabs=T, caption = "Comparación de modelos",escape = F)%>%
  ...
  add_footnote( c("Para hacer el probit comparable solamente se muestra el Efecto Marginal Promedio con respecto a la variable Black"), notation = "symbol")
...
## Imagina  que  estratificas  por:  (i)  sexo  del  aplicante  (hombre  o  mujer),  (ii)  ciudad donde se postula al trabajo (Chicago o Boston) e (iii) industria de la empresa quepublico  el  puesto  (ver  el  pdf  que  indica  las  industrias  disponibles)  [Ejemplo:   unposible estrato sería hombres aplicantes a trabajos en Chicago en la industria manufacturera]. 
...
$$V( \tau_{g})=\frac{S^2_{t,g}}{N_{t,g}}+\frac{S^2_{c,g}}{N_{c,g}} $$
y la varianza total después de estratificar:

$$V_{strat}( \tau)=\sum_{g=1}^G V( \tau_{g}).(\frac{N_g}{N})^2$$
...
# varianza
(varianza_total_por_estrato<-sum(neyman_strat$variance_group*((neyman_strat$weights)^2)))

# desviación estandar
(desviacion_tota_estratificado<-varianza_total_por_estrato^.5)
